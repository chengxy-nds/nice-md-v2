...
class CodeAdapter {
  ...
  async getCookieValue(url, name) {
    if (typeof globalThis !== 'undefined' && globalThis.__NICEMD_GET_COOKIE__) {
      try {
        const val = await globalThis.__NICEMD_GET_COOKIE__(url, name);
        if (val) return val;
      } catch (e) {}
    }
    if (typeof window !== 'undefined' && window.electronAPI && typeof window.electronAPI.getCookie === 'function') {
      try {
        const val = await window.electronAPI.getCookie(url, name);
        if (val) return val;
      } catch (e) {}
    }
...
class CnblogsAdapter extends CodeAdapter {
  ...
  async publish(article) {
    ...
    const editRes = await this.fetch('https://i.cnblogs.com/posts/edit');
    const editHtml = await editRes.text().catch(() => '');

    // Allow set-cookie to settle in cookie jar
    await new Promise(resolve => setTimeout(resolve, 120));

    // 2. Read XSRF token precisely from the posts API endpoint URL
    let xsrfToken = await this.getCookieValue('https://i.cnblogs.com/api/posts', 'XSRF-TOKEN') ||
                    await this.getCookieValue('https://i.cnblogs.com/posts/edit', 'XSRF-TOKEN') ||
                    await this.getCookieValue('https://i.cnblogs.com', 'XSRF-TOKEN') ||
                    await this.getCookieValue('https://cnblogs.com', 'XSRF-TOKEN');

    if (!xsrfToken && editHtml) {
      const tokenMatch = editHtml.match(/name="__RequestVerificationToken"\s+value="([^"]+)"/) ||
                         editHtml.match(/"xsrfToken":\s*"([^"]+)"/) ||
                         editHtml.match(/"antiForgeryToken":\s*"([^"]+)"/) ||
                         editHtml.match(/XSRF-TOKEN=([^;"]+)/);
    ...
    const body = {
      ...
      url: null,
      postBody: processedMarkdown || article.html || '',
      categoryIds: [],
      collectionIds: [],
    ...
    if (!response.ok) {
      ...
      let errMsg = `博客园 API 返回 HTTP ${response.status}`;
      try {
        const errJson = JSON.parse(errText);
        if (errJson.message || errJson.errors || errJson.title) {
          errMsg += ` (${errJson.message || JSON.stringify(errJson.errors) || errJson.title})`;
        }
      } catch (e) {
...
class SegmentfaultAdapter extends CodeAdapter {
  ...
  async getSessionToken() {
    try {
      // 1. Check direct cookies (token, PHPSESSID, SHARESESSID)
      const tokenCookie = await this.getCookieValue('https://segmentfault.com', 'token') ||
                          await this.getCookieValue('https://segmentfault.com', 'PHPSESSID') ||
                          await this.getCookieValue('https://segmentfault.com', 'SHARESESSID') ||
                          await this.getCookieValue('segmentfault.com', 'token') ||
                          await this.getCookieValue('segmentfault.com', 'PHPSESSID') ||
                          await this.getCookieValue('segmentfault.com', 'SHARESESSID');
      ...
      const settingsHtml = await settingsRes.text().catch(() => '');
...
if (typeof self !== 'undefined') {
  self.publishAdapters = publishAdapters;
}
if (typeof window !== 'undefined') {
  window.publishAdapters = publishAdapters;
}
if (typeof globalThis !== 'undefined') {
  globalThis.publishAdapters = publishAdapters;
}
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { publishAdapters };
}
